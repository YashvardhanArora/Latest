...
export default function ReportsPage() {
  ...
  const exportAnalyticsReport = async () => {
    try {
      ...
      // Filter resolved complaints within date range
      const resolvedComplaints = complaints.filter(complaint => {
        const resolutionDate = complaint.dateOfResolution 
          ? new Date(complaint.dateOfResolution) 
          : complaint.dateOfClosure 
          ? new Date(complaint.dateOfClosure) 
          : null;
      ...
      toast({
        ...
        description: `Found ${resolvedComplaints.length} resolved complaints in the selected date range.`,
      });
    } catch (error) {
      toast({
        title: "Export Failed",
